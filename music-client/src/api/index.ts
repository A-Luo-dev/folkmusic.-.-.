import { ky } from "element-plus/lib/locale";
...
const HttpManager = {
  ...
  // =======================> 用户 API 完成
  // 登录
  signIn: ({username,password,code,key}) => post(`user/login/status`, {username,password,code,key}),
...
// 记录点击
addUserSongClick: (params: { userId: number; songId: number }) =>
  post(`user_song_click/add`, params),

// 获取最近点击
getUserRecentClick: (userId: number) =>
  get(`user_song_click/recent/${userId}`),

// 搜索记录相关API
addSearchHistory: (keyword) => post(`search-history/add`, { keyword }),
getSearchHistory: () => get(`search-history/list`),
clearSearchHistory: () => deletes(`search-history/clear`),
removeSearchHistory: (keyword) => deletes(`search-history/remove`, { keyword }),
  ...
  getAllRankList: () => get("rankSong/list"),
  getMenuId: ({ id }: { id: number }) => get(`rankSong/detail?rankSongId=${id}`),
